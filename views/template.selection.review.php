...
if ($eligibleIds !== [] && !empty($data['can_prepare'])) {
	...
	$page
		...
		->addItem(new CTag('p', true, _(
			'Preparation runs the full safety analysis for every selected candidate, one template per HTTP request. For low-risk candidates it may create or refresh a persistent rollback backup and then run a fresh preflight. It does not import Zabbix configuration.'
		)))
		->addItem($form);
}
