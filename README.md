...
A fixed laboratory snapshot is published as branch `release/0.1.0-beta.43` after automation validation. Formal tag/GitHub Release automation now exists, but publishing a production-grade release remains gated by field validation and the still-unselected project license.
...
git fetch origin release/0.1.0-beta.43
git checkout -B release/0.1.0-beta.43 origin/release/0.1.0-beta.43
cat VERSION
git rev-parse HEAD
```
...
Confirm version **0.1.0-beta.43**, enable the module and open:
