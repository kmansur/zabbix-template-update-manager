...
final class TemplateControlledUpdateService {
	...
	private function assertCandidateMatchesPreflight(array $candidate, array $preflight): void {
		...
		foreach (['commit', 'path', 'source_sha256', 'content_sha256', 'uuid', 'name', 'technical_name', 'vendor_name', 'vendor_version'] as $field) {
		...
		if (!preg_match('/^[a-f0-9]{64}$/', (string) ($candidate['source_sha256'] ?? ''))
				|| !preg_match('/^[a-f0-9]{64}$/', (string) ($candidate['content_sha256'] ?? ''))
