...
assertBatchContract(strpos($prepareBehavior, "CCsrfTokenHelper::get('ztum.templates.prepare_one')") !== false,
	'Queue requests must use the per-template action CSRF token.');
assertBatchContract(strpos($prepareBehavior, 'fetch(config.prepareOneUrl') !== false,
	'Batch preparation view must drive one bounded HTTP request per template.');
assertBatchContract(strpos($prepareBehavior, 'for (let index = 0; index < config.templateIds.length; index++)') !== false,
	'Batch preparation queue must be sequential and bounded.');
assertBatchContract(strpos($prepareBehavior, 'Stop after current template') !== false,
...
assertBatchContract(strpos($prepareBehavior, "result.status !== 'updated'") !== false
		&& strpos($prepareBehavior, 'labels.not_attempted') !== false,
	'Request-bounded execution must stop on first non-success and mark later templates not attempted.');
assertBatchContract(strpos($prepareBehavior, "category === 'ready' && !isValidEvidence(evidence)") !== false
		&& strpos($prepareBehavior, "reason = 'invalid_preflight_evidence'") !== false,
	'Browser batch state must fail closed when a server-reported Ready row lacks valid SHA-256 evidence.');
assertBatchContract(strpos($prepareBehavior, 'const executionCount = readyEvidence.size + selectedReviewed;') !== false
		&& strpos($prepareBehavior, 'fullyPrepared && executionCount > 0') !== false,
	'Completed plans must enable execution for Ready candidates plus explicitly selected reviewed overrides.');
assertBatchContract(strpos($prepareBehavior, "if (category === 'ready') {") !== false
		&& strpos($prepareBehavior, 'readyEvidence.set(templateId, evidence);') !== false,
	'Only Ready candidates may enter the request-bounded execution set.');
assertBatchContract(strpos($prepareBehavior, 'Unavailable — no executable templates selected.') !== false
		&& strpos($prepareBehavior, 'Available — {ready} Ready + {review} selected reviewed template(s).') !== false
		&& strpos($prepareBehavior, 'Unavailable — preparation stopped.') !== false,
	'Batch execution availability must be explicit for Ready, selected reviewed, zero-executable and stopped plans.');
assertBatchContract(strpos($prepareBehavior, "setStateText('ztum-batch-exec-status', labels.execution_none, 'muted');") !== false,
	'Zero-Ready plans must synchronize the execution summary status instead of leaving Waiting for preparation.');
assertBatchContract(strpos($prepareBehavior, 'Retry failed preparation') !== false
		&& strpos($prepareBehavior, 'requestFailures = new Set()') !== false
		&& strpos($prepareBehavior, 'retryFailedPreparation = async () =>') !== false,
	'Failed per-template preparation must expose an explicit operator-controlled retry path.');
assertBatchContract(
	strpos($prepareBehavior, "'HTTP ' + response.status + ' after ' + elapsedSeconds + 's'") !== false
		&& strpos($prepareBehavior, "response.headers.get('server')") !== false
		&& strpos($prepareBehavior, "response.headers.get('cf-ray')") !== false,
	'Preparation transport failures must include elapsed-request timing and bounded gateway identity diagnostics.'
);
assertBatchContract(
	strpos($prepareBehavior, 'maxHistoricalContinuationRequests') !== false
		&& strpos($prepareBehavior, "item?.reason === 'historical_baseline_time_budget_reached'") !== false
		&& strpos($prepareBehavior, 'prepareUntilSettled') !== false,
	'Long historical baseline scans must continue across bounded preparation requests instead of one proxy-long request.'
);
assertBatchContract(strpos($prepareBehavior, "'compareUrl' => \$compareUrl") !== false
		&& strpos($prepareBehavior, "link.href = config.compareUrl + '&templateid=' + encodeURIComponent(templateId);") !== false
		&& strpos($prepareBehavior, "link.textContent = labels.review_details;") !== false,
	'Manual-review rows must retain a direct Review details link to the individual comparison flow.');
assertBatchContract(strpos($prepareBehavior, "new CCheckBox('review_select['.\$templateId.']', '1')") !== false
		&& strpos($prepareBehavior, "->setId('ztum-review-select-'.\$templateId)") !== false
		&& strpos($prepareBehavior, 'document.createElement(\'input\')') === false
		&& strpos($prepareBehavior, 'setReviewedSelection(templateId, category, manualState);') !== false
		&& strpos($prepareBehavior, 'reviewEvidence.set(templateId') !== false,
	'Eligible Manual review rows must use a server-rendered native Zabbix CCheckBox in the leading selection column.');
assertBatchContract(strpos($prepareBehavior, 'No unattended Ready templates. Select eligible Manual review rows below') !== false
...
assertBatchContract(strpos($prepareBehavior, "new CButton('ztum-reviewed-select-all', _('Select all eligible'))") !== false
		&& strpos($prepareBehavior, "new CButton('ztum-reviewed-clear-all', _('Clear reviewed selection'))") !== false
		&& strpos($prepareBehavior, "byId('ztum-reviewed-select-all').addEventListener('click'") !== false
		&& strpos($prepareBehavior, "byId('ztum-reviewed-clear-all').addEventListener('click'") !== false,
	'Large reviewed batches must provide explicit Select all eligible and Clear selection controls.');
assertBatchContract(strpos($prepareBehavior, 'let autoSelectReviewed = false;') !== false
		&& strpos($prepareBehavior, 'checkbox.checked = autoSelectReviewed;') !== false
		&& strpos($prepareBehavior, 'autoSelectReviewed = true;') !== false
		&& strpos($prepareBehavior, 'autoSelectReviewed = false;') !== false,
...
assertBatchContract(strpos($prepareBehavior, 'selectAll.disabled = executionStarted || retryInProgress || (fullyPrepared && eligible.length === 0);') !== false
		&& strpos($prepareBehavior, "selectAll.textContent = labels.select_all_reviewed + ' (' + eligible.length + ')'") !== false
		&& strpos($prepareBehavior, "clearAll.textContent = labels.clear_all_reviewed + ' (' + selected + ')'") !== false,
...
assertBatchContract(strpos($prepareBehavior, 'Waiting for preparation to finish — {completed} of {total} completed.') !== false,
	'Execution state must explain that updates wait for full preparation while selection can already be made.');
assertBatchContract(strpos($prepareBehavior, 'setOnDocumentReady()') !== false,
	'Batch behavior must initialize through the native Zabbix document-ready script lifecycle.');
assertBatchContract(strpos($prepareBehavior, "'success' => ZBX_STYLE_GREEN") !== false
		&& strpos($prepareBehavior, "'warning' => ZBX_STYLE_ORANGE") !== false
...
$combined = $prepare.$prepareOne.$update.$updateOne.$prepareView.$prepareAsset.$updateView;
foreach ([
